...
TEST_PATH = Path("ml/data/cicids2018_test.csv")
MODEL_PATH = Path("ml/saved_models/cicids2018_live_compatible_model.pkl")
LABEL_ENCODER_PATH = Path("ml/saved_models/cicids2018_live_compatible_label_encoder.pkl")
FEATURE_COLUMNS_PATH = Path("ml/saved_models/cicids2018_live_compatible_feature_columns.json")
RESULTS_DIR = Path("ml/results")
REPORT_PATH = RESULTS_DIR / "cicids2018_evaluation_report.md"
...
def main():
    ...
    X_test = prepare_cicids2018_features(df)
    if X_test.empty:
        raise ValueError("Test dataset contains no rows.")

    df = df.loc[X_test.index]
    y_test_raw = df[LABEL_COLUMN].apply(normalize_cicids2018_label)
    ...
    report = (
        "# CIC IDS 2018 Model Evaluation Report\n\n"
        "## Dataset\n\n"
