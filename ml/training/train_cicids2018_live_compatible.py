...
DATASET_PATH = Path("ml/data/cicids2018_train.csv")
...
def _prepare_live_compatible_dataset(df: pd.DataFrame):
    ...
    X = prepare_cicids2018_features(df)

    y_raw = df[LABEL_COLUMN].apply(normalize_cicids2018_label).astype(str)
    encoder_path = MODEL_OUTPUT_DIR / "cicids2018_live_compatible_label_encoder.pkl"
...
def train():
    ...
    model_path = MODEL_OUTPUT_DIR / "cicids2018_live_compatible_model.pkl"
    label_encoder_path = MODEL_OUTPUT_DIR / "cicids2018_live_compatible_label_encoder.pkl"
    feature_columns_path = MODEL_OUTPUT_DIR / "cicids2018_live_compatible_feature_columns.json"
    report_path = MODEL_OUTPUT_DIR / "cicids2018_live_compatible_benchmark_report.json"
    metadata_path = MODEL_OUTPUT_DIR / "cicids2018_live_compatible_training_metadata.json"
